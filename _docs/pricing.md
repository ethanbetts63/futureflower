# Pricing Page - Textual Representation

## SEO Information
*   **Title:** Flower Subscription Pricing Plans | FutureFlower
*   **Description:** Simple, transparent pricing for flower delivery and subscriptions. Set your budget, choose your dates, and local florists handle the rest.
*   **Canonical Path:** /pricing
*   **Service Schema:**
    *   Name: FutureFlower Flower Delivery
    *   Description: Personalized flower delivery and subscription service. Choose a budget tier, set your dates, and local florists handle the rest.
    *   Provider: FutureFlower

## Hero Section (HeroV2)
*   **Title:** More flowers. Fewer fees.
*   **Subtitle:** Give florists a budget and preferences. No catalogs, just the freshest, seasonal flowers.

## Florist Advantage Section
**Title:** Why we don't have a catalog.
**Subtitle:** The Florist's Choice Advantage

**Text:**
Most online flower deliveries force local florists to recreate rigid "recipes" from a picture. This forces them to use cheaper, hardier flowers that survive long transit, rather than the fresh, seasonal blooms they actually want to use.

**How FutureFlower works instead:**
1.  **Choose the occasion:** Let the florist know the vibe — Romantic, Sympathy, Birthday, and more.
2.  **Add your preferences:** Leave specific notes if you want — "She loves peonies," or "Please no lilies."
3.  **Set your exact budget:** Select your pricing tier or enter a custom amount.

## Pricing Tiers Section
**Title:** How much does it cost?
**Subtitle:** Price Guidelines
**Text:** As much as you want. Select a budget tier or custom amount and we'll do the rest.

### Tiers (IMPACT_TIERS)
1.  **The Signature**
    *   Price: $65
    *   Description: A beautiful, seasonal arrangement. Perfect for any occasion.
2.  **The Statement** (Badge: Most Popular)
    *   Price: $125
    *   Description: Lush, premium flowers designed to make an impression. Our most popular choice.
3.  **The Grand Gesture**
    *   Price: $300
    *   Description: A show-stopping display of luxury blooms for life's biggest milestones.

## Comparison Section
**Title:** The Secret Big Platforms Don't Tell You
**Subtitle:** Same price, more flowers. By cutting the fees big networks charge, we ensure more of your money ends up in the bouquet.

### Get More Bloom for Your Buck
(Bar Chart Visual)

### Then how do you make money?
Where our competitors slip in a 20–40% commission, monthly fees, and delivery cuts, we charge a simple 15%. You get more, the florist gets more, and we get customers that keep coming back.

### How Big Online Platforms Treat Florists (Table)
| Feature | Big Networks | FutureFlower |
| :--- | :--- | :--- |
| **Commission** | 20–40% per order | **15%** |
| **Monthly Fees** | Often $100–$200+ monthly | **$0** |
| **Order Rejection** | $10–$25 penalty for rejecting | **$0** |
| **Delivery Fee** | Often retain delivery margin | **Fully Paid** |
| **Product Rules** | Exact vase + stem counts | **Florist designs freely** |
| **Branding** | Network branding required | **Florist branding encouraged** |

## Questions? We have answers. 
### Can I request specific flowers?
Yes. When placing your order you can leave notes like "Loves lilies" or "No pink," and the florist will accommodate your preferences based on seasonal availability. Because we don't enforce a rigid recipe, your florist has the freedom to honour your requests.

### Is delivery really included?
Yes. The budget you type is the exact amount charged to your card. There is no delivery fee added at checkout — it is built into the budget from the start.

### Can I change my order details later?
Yes. You can adjust the most of the details for any upcoming scheduled delivery up to 14 days before the delivery date. Simply log in to your dashboard, open the plan, and update the budget. Changes apply to that delivery and all future ones on that plan.
