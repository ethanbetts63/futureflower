...
### Completed in Third Pass (Phases 4 & 5: Webhook Rewrite + Model Collapse)

- **Model collapse.** `UpfrontPlan` and `SubscriptionPlan` are deleted. `years`, `stripe_subscription_id`, and `subscription_message` now live directly on `OrderBase`, which is the single concrete order table. `billing_mode` is a required field (`default='one_time'`), no longer an optional MTI-fallback. `OrderBase.get_child_instance()` is gone — nothing needs it anymore. Migration `events/0005` handles the collapse (no data-preservation logic was needed; the dev DB was rebuilt from scratch per explicit sign-off, since this was pre-launch data).
- **Webhook dispatch on `billing_mode`.** `payments/utils/webhook_handlers.py` no longer branches on Stripe metadata `item_type`. `handle_payment_intent_succeeded` reads `payment.order.billing_mode` directly and dispatches to `_activate_order` / `_create_first_event` / `_create_projected_events` / `_create_stripe_subscription_for_order` helpers, matching the doc's original target design exactly. `handle_invoice_payment_succeeded`, `handle_setup_intent_succeeded`, and `handle_subscription_deleted` now query `OrderBase` directly instead of `SubscriptionPlan`.
- **`make_recurring` mutates in place.** The `OrderViewSet.make_recurring` action (previously a migration bridge that copied a draft into a new `SubscriptionPlan` row) now just sets `billing_mode='recurring'` and the recurring fields directly on the same order row — the "direct mutation" target from Phase 2 is done.
- **Checkout simplified.** `OrderViewSet.checkout` no longer needs to resolve billing_mode from a child type or fake an `item_type` for backward compatibility — it reads `order.billing_mode` straight from the row.
- **Editing an active order is gone by design, not by omission.** Per product decision: there is no path to change budget/frequency/structure on an already-paid order. `SubscriptionStructureEditor`, its form/card components, and the `/dashboard/subscription-plans/[id]/edit-structure` route are deleted. The dashboard's "Danger Zone" now tells the customer to cancel and place a new order instead. Recipient and preference edits (non-billing-affecting) are still supported. `UPFRONT_PLAN_MODIFY` (upfront plan price changes) and its price-calculation utility (`upfront_price_calc.py`) are deleted outright — it had no live caller.
- **`OrderViewSet` gained a `cancel` action**, replacing the old separate `UpfrontPlanViewSet.cancel` / `SubscriptionPlanViewSet.cancel` (Stripe subscription cancellation, event/notification cleanup, admin alert for already-ordered events — all unified into one implementation keyed off `billing_mode`).
- **Backend cleanup:** deleted `upfront_plan_view.py`, `subscription_plan_view.py`, `get_or_create_inactive_plan_view.py`, their serializers, `create_payment_intent.py`, `create_subscription_view.py`, and `upfront_price_calc.py`. Updated every remaining consumer (admin serializers/views, partner discount validation, user anonymization, Django admin registration) to query `OrderBase`/`billing_mode` instead of the deleted child models.
- **Frontend cleanup:** deleted `api/upfrontPlans.ts`, `api/subscriptionPlans.ts`, `api/singleDeliveryPlans.ts`, `api/payments.ts`, and ~20 now-dead types/components that only existed to serve the old plan-type split (`Plan`, `UpfrontPlan`, `SubscriptionPlan` and their `Partial*` variants, `UnifiedPlanTable`'s sibling tables, `OrderSummaryCard`, `MessagesCard`, `DeliveryDatesCard`, `PlanStructureCard`/Form, etc. — confirmed dead via grep before deletion, not guessed). Every dashboard page (upfront and subscription management) now reads/writes through the single `orders.ts` client. `PaymentInitiatorButton` shrank from four branches to one (`startCheckout(orderId)`).
- Verified end-to-end: `manage.py check`, full backend suite (578 tests passing), `tsc --noEmit -p tsconfig.app.json` clean, and `next build` production build succeeds.

### Completed in Fourth Pass (Dead Code / DRY Cleanup)

- **`prepaid` billing_mode removed entirely.** Nothing in the live product created prepaid multi-year orders — it was speculative generality left over from the old `UpfrontPlan` shape. Dropped from `BILLING_MODE_CHOICES`, dropped the `years` field from `OrderBase`, deleted `_create_projected_events` from the webhook and `events/utils/delivery_dates.py` (`calculate_projected_delivery_dates`, `frequency_to_deliveries_per_year`) outright. `billing_mode` is now a true `one_time` / `recurring` binary end to end (model, serializer, webhook, admin filters, frontend types).
- **Dead SetupIntent handling removed.** Confirmed via grep that no checkout path anywhere in the app ever creates a Stripe SetupIntent — `handle_setup_intent_succeeded`/`handle_setup_intent_failed` and their webhook routing are gone, as is the `intentType: 'setup'` branch in `CheckoutForm.tsx` and the SetupIntent-handling half of `PaymentStatusPage.tsx`. `CheckoutForm` now only knows how to confirm a PaymentIntent.
- **Test factories unified.** `UpfrontPlanFactory` and `SubscriptionPlanFactory` were two ~90%-identical factories left over from the MTI days. Collapsed into a single `events/tests/factories/order_factory.py::OrderFactory(billing_mode=...)`, with every call site across the suite (~24 files) mechanically updated.
- **Dashboard route trees unified.** `/dashboard/upfront-plans/[planId]/...` and `/dashboard/subscription-plans/[planId]/...` were two parallel route trees that had converged on identical behavior once both sat behind the same `Order` API — the only difference left was which summary component they rendered. Collapsed into one `/dashboard/orders/[planId]/{overview,edit-recipient,edit-preferences,edit-structure,cancel}` tree, backed by one set of page components in `page_components/user_dashboard/order_management/`.
- **`UpfrontSummary` and `SubscriptionSummary` merged into `OrderSummary`.** The two display components overlapped in everything except the schedule section and danger-zone copy; `OrderSummary` branches on `billing_mode` internally for those two spots and serves both the `ordering` (pre-payment review) and `management` (dashboard) contexts that `UpfrontSummary` used to serve alone.
