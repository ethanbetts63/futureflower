...
- Replace the legacy single-delivery/upfront-plan/subscription-plan route naming (`/single-delivery-flow/...`, `/dashboard/upfront-plans/...`, `/dashboard/subscription-plans/...`) with neutral `/order/[orderId]/...` naming, and collapse the two dashboard management route trees into one now that they're both backed by the same `Order` API.
