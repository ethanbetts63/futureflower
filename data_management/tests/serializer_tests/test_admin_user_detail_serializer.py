...
@pytest.mark.django_db
def test_admin_user_detail_serializer_plans(settings):
    """
    Test that plans (both upfront and subscription) are correctly serialized and included.
    """
    # Pin the fee settings: the configured threshold is a business value that
    # moves (currently $1, so no order ever pays a fee).
